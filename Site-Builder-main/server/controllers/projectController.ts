...
export const makeRevision = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const codeGenerationResponse = await openai.chat.completions.create({
            ...
            messages: [
                ...
                },
                {
                    ...
                    Now apply the following changes : ${enhancedPrompt}`
                }
            ]
        })
        ...
        await prisma.conversation.create({
            data: {
                role: 'assistant',
                content: `Here is the updated website code, You can now preview it.`,
                projectId
            }
        })
        ...
        await prisma.websiteProject.update({
            ...
            data: {
                current_code: code.replace(/```[a-z]*n?/g, '')
                    .replace(/```/g, '')
                    .trim(),
...
// Controller to a previous version
export const rollBackToVersion = async (req: Request, res: Response) => {
    ...
    try {
        ...
        //     return res.status(400).json({ message: "Please provide a valid project ID and version ID" });
        // }
        ...
        res.json({ message: "Rolled back to previous version successfully" });
...
// Conrtoller Function for getting Published Projects
...
// Controller Function for Saving Project Code
