            ...
            <div className='flex flex-col h-full'>
                ...
                <div className='flex-1 overflow-y-auto no-scrollbar px-3 flex flex-col gap-4'>
                    {[...project.conversation, ...project.versions].sort((a, b) =>
                        new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()).map((message) => {
                                ...
                                return (
                                    <div key={msg.id} className={`flex items-start gap-3 ${isUser ? "justify-end" : "justify-start"
                                        }`}>
                                        {
                                            !isUser && (
                                                <div className={`w-8 h-8 rounded-full
                                                            bg-linear-to-br from-indigo-700 to-indigo-600
                                        ...
                                        <div className={`max-w-[80%] p-2 px-4 rounded-2xl
                                                        shadow-sm text-sm mt-5 leading-relaxed ${isUser ? "bg-linear-to-r from-indigo-500 to-indigo-600 text-white rounded-tr-none"
                                                : "text-white rounded-tl-none bg-gray-800"
                                    ...
                                    <div key={ver.id} className='w-4/5 mx-auto my-2 p-3 rounded-xl bg-gray-800 text-gray-100 shadow flex flex-col gap-2'>
                                        ...
                                        <div className='flex items-center justify-between'>
                                            ...
                                            <Link target='_blank' to={`/preview/${project.id}/${ver.id}`}>
                                                <EyeIcon className='size-6 p-1 bg-gray-700 hover:bg--indigo-500 transitioin-colors rounded' />
                                            </Link>
                                        </div>
                                    </div>
                                )
